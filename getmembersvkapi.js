$(document).ready(function() {
    ...
    function getMembers20k(group_id, members_count) {
      var code =  'var members = API.groups.getMembers({"group_id": ' + group_id + ', "v": "5.27", "sort": "id_asc", "count": "1000", "fields": "sex, bdate, city, country", "offset": ' + membersGroups.length + '}).items;' // делаем первый запрос и создаем массив
          ...
          + '{'
            + 'members = members + "," + API.groups.getMembers({"group_id": ' + group_id + ', "v": "5.27", "sort": "id_asc", "count": "1000", "fields": "sex, bdate, city, country", "offset": (' + membersGroups.length + ' + offset)}).items;' // сдвиг участников на offset + мощность массива
      ...
      VK.Api.call("execute", {code: code}, function(data) {
        if (data.response) {
          console.log("ASDSA" + JSON.parse("[" + data.response.items + "]"));
